...
createMatchItCovMatrix <- function(seqs, ...){
	x.1 <- mclapply(1:length(seqs),function(i)  countOccurences(i,seqs[[i]]),mc.cores=mc.cores)
	x.2 <- mclapply(1:length(seqs),function(i)  countOccurences(i,seqs[[i]],wordsize=2),mc.cores=mc.cores)
...
runFimo <- function(gr, pwm_db=NA, genome=genome, type = "encode", ...){
	...
	write.fasta(sequences = seqs, names = dgf.names, file.out = paste0("../ePPI/data/target_",type,".fasta"))


	fasta_get_markov_cmd <- '/home/gaga/tools/meme/bin/fasta-get-markov '

	cmd_line <- paste0(fasta_get_markov_cmd,paste0("../ePPI/data/target_",type,".fasta"), paste0(" ../ePPI/data/bg_",type,".txt"))

	res <- system(cmd_line, intern = TRUE)

	fimo_cmd <- '/home/gaga/tools/meme/bin/fimo'
	if(is.na(pwm_db))
		pwm_db <- '/home/gaga/tools/meme/db/motif_databases/HUMAN/HOCOMOCOv11_core_HUMAN_mono_meme_format.meme'
	out_folder <- paste0('--oc ../ePPI/data/fimo_out_',type)
	bgfile <- paste0('--bgfile ../ePPI/data/bg_',type,'.txt')
	fasfile <- paste0("../ePPI/data/target_",type,".fasta")
	...
	fimo.gr <- import(con = paste0("../ePPI/data/fimo_out_",type,"/fimo.gff"), format = "GFF")
	...
	fimo.gr$TF <- unlist(mclapply(fimo.gr$Name,function(x) unlist(strsplit(x,"_"))[1],mc.cores=mc.cores))
